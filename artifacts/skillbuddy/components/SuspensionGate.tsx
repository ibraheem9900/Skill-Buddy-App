...
export default function SuspensionGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.dark.background, paddingTop: insets.top + 40 }]}>
      ...
      <Text style={styles.title}>{t('susp_title')}</Text>
      <Text style={styles.message}>{t('susp_msg')}</Text>
      <View style={styles.daysCard}>
        <Text style={styles.daysValue}>{daysLeft}</Text>
        <Text style={styles.daysLabel}>
          {t(daysLeft !== 1 ? 'susp_days_plural' : 'susp_days', { n: daysLeft })} {t('susp_remaining')}
        </Text>
      </View>
      <Text style={styles.hint}>{t('susp_hint')}</Text>
      <TouchableOpacity style={styles.switchBtn} onPress={toggleRole}>
        <Text style={styles.switchText}>{t('susp_switch')}</Text>
      </TouchableOpacity>
    </View>
  );
}
